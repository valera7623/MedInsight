/** Dashboard and shared page logic (requires js/auth.js, js/toast.js). */
...
function setupDocumentActions() {
  ...
  docsEl.addEventListener('click', async (event) => {
    ...
    try {
      ...
      notifyError(err.message || 'Ошибка работы с документом');
    } finally {
...
async function testWebhook(id) {
  ...
  const data = await res.json();
  if (data.delivered) notifySuccess('Вебхук доставлен ✓');
  else notifyError('Доставка не удалась (проверьте URL)');
  loadWebhooks();
}
...
async function exportPatientPdf(patientId) {
  ...
  if (!res.ok) {
    const data = await res.json();
    notifyError(data.detail || 'Ошибка экспорта');
    return;
  }
...
async function exportPatientDocx(patientId, btn = null) {
  ...
  try {
    ...
    notifyError(err.message || 'Ошибка экспорта DOCX');
  } finally {
    if (btn) {
      btn.disabled = false;
...
async function exportToExcel(entity, filters = {}, columns = [], btn = null) {
  ...
  try {
    ...
    notifyError(err.message || 'Ошибка экспорта');
  } finally {
...
async function pollExportDownload(downloadUrl, entity) {
  ...
  notifyError('Экспорт занимает слишком много времени. Попробуйте позже по ссылке скачивания.');
}
...
async function setupPatientFhir(patientId) {
  const capRes = await apiFetch('/api/auth/capabilities');
  const cap = await capRes.json().catch(() => ({}));
  if (!cap.fhir_enabled) return;
  document.getElementById('export-fhir-btn')?.classList.remove('hidden');
  document.getElementById('import-fhir-btn')?.classList.remove('hidden');
  document.getElementById('export-fhir-btn')?.addEventListener('click', async () => {
    try {
      const res = await apiFetch(`/api/fhir/export/patient/${patientId}`);
      const data = await res.json();
      if (!res.ok) throw new Error(formatApiError(data.detail) || 'Ошибка экспорта');
      const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = `patient-${patientId}-fhir.json`;
      a.click();
      notifySuccess('FHIR bundle экспортирован');
    } catch (e) { notifyError(e.message); }
  });
  document.getElementById('import-fhir-btn')?.addEventListener('click', () => {
    document.getElementById('import-fhir-file')?.click();
  });
  document.getElementById('import-fhir-file')?.addEventListener('change', async (ev) => {
    const file = ev.target.files?.[0];
    if (!file) return;
    try {
      const text = await file.text();
      const payload = JSON.parse(text);
      const res = await apiFetch('/api/fhir/import/bundle', { method: 'POST', body: JSON.stringify(payload) });
      const data = await res.json();
      if (!res.ok) throw new Error(formatApiError(data.detail) || 'Ошибка импорта');
      notifySuccess('FHIR bundle импортирован');
...
function initPatientPage() {
  ...
  fetchCurrentUser()
    .then(async () => {
      applyWriteRestrictions();
      await setupPatientFhir(patientId);
