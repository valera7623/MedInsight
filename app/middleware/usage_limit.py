...
# Don't email the same tenant more than once per this many seconds.
_LIMIT_EMAIL_THROTTLE_SECONDS = 24 * 3600
...
def _should_send_limit_email(tenant_id: int) -> bool:
    """Throttle limit-exceeded emails via Redis; fail-open if Redis is down."""
    try:
        ...
        if client is None:
            return True
        key = f"limit_email:{tenant_id}"
        # SET NX returns True only the first time within the TTL window.
        return bool(client.set(key, "1", nx=True, ex=_LIMIT_EMAIL_THROTTLE_SECONDS))
...
async def _notify_limit_exceeded(tenant_id: int) -> None:
    try:
        if not _should_send_limit_email(tenant_id):
            return
        ...
        logger.warning("Limit-exceeded email failed for tenant %s: %s", tenant_id, exc)
